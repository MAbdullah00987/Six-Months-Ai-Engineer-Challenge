...
#Goal: Master SVD, one of the most powerful matrix factorizations in ML.

#Mathematics for Machine Learning, Chapter 4: Section on SVD
#BITS Pilani Course: SVD lectures
#Understand the relationship between SVD and eigendecomposition

#Practice Projects
#Image Compression with SVD: Use SVD to compress an image by keeping only the top k singular values. Show reconstructions with k = 5, 20, 50, 100 and compare file sizes.
#Recommender System with SVD: Implement a basic movie recommender system using SVD on a user-item rating matrix (use MovieLens or create synthetic data).

#Key Concepts to Master
#SVD decomposition: U, Σ, V^T
#Singular values vs eigenvalues
#Low-rank approximation
#Applications: compression, denoising, collaborative filtering
...
print("PROJECT 1: PAGERANK ALGORITHM (SIMPLIFIED)")
...
# PART 8: VISUALIZE WEB GRAPH (WITHOUT NETWORKX)
...
# Manual graph layout (circular arrangement)
def circular_layout(n, radius=2):
    """Create circular layout for n nodes"""
...
# Draw nodes
node_sizes = pagerank_scores * 1000  # Scale for visibility
colors = plt.cm.YlOrRd(pagerank_scores / pagerank_scores.max())

for i, page in enumerate(pages):
    x, y = pos[i]
    circle = Circle((x, y), radius=node_sizes[i]/500, 
                   color=colors[i], ec='black', linewidth=2, zorder=10)
    ax.add_patch(circle)
    ax.text(x, y, page, ha='center', va='center', 
...
ax = axes[0, 1]
colors_bar = plt.cm.YlOrRd(pagerank_scores / pagerank_scores.max())
bars = ax.barh(ranking_df['Page'], ranking_df['PageRank'], color=colors_bar, 
...
for i, (page, score) in enumerate(zip(ranking_df['Page'], ranking_df['PageRank'])):
    ax.text(score + 0.002, i, f'{score:.4f}', va='center', ha='left', 
...
# PART 11: DETAILED LINK ANALYSIS


print("\n" + "="*80)
print("PART 11: DETAILED LINK ANALYSIS")
print("="*80)
